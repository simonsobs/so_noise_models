...
class SOLatType:
    ...
    def precompute(self, N_tubes, N_tels=1):
        ...
        self.Tatmos_C = (
            get_atmosphere_C(self.bands, self.atm_version, el=self.el) * self.FOV_mod
        )
    ...
    def get_noise_curves(
        self, f_sky, ell_max, delta_ell, deconv_beam=True, full_covar=False
    ):
...
SO_el_noise_func_params = {
    "threshold": {"valid": ("only", 50.0)},
    "baseline": {
        "valid": (25.0, 70.0),
        "bands": [20, 27, 39, 93, 145, 225, 280],
        "coeffs": [
            (178.59719595, 33.72945249),  # From B. Racine & D. Barkats.
            (0.87, 0.09),  # From Carlos Sierra and J. McMahon vvv
            (0.64, 0.25),
            ((0.80 + 0.74) / 2, (0.14 + 0.19) / 2),
            ((0.76 + 0.73) / 2, (0.17 + 0.19) / 2),
            (0.58, 0.30),
            (0.49, 0.36),
        ],
    },
    "goal": {
        ...
        "coeffs": [
            (178.59719595, 33.72945249),  # From B. Racine & D. Barkats.
            (0.85, 0.11),  # From Carlos Sierra and J. McMahon vvv
            (0.65, 0.25),
            ((0.76 + 0.69) / 2, (0.17 + 0.22) / 2),
            ((0.73 + 0.69) / 2, (0.19 + 0.22) / 2),
            (0.55, 0.32),
            (0.47, 0.37),
        ],
    },
...
class SOLatV3(SOLatType):
    ...
    def __init__(
        ...
        assert (
            sensitivity_mode in sens_modes.values()
